...
export default class GardenLazyCardController extends Controller {
  static override values = {
    ...
    entryCollection: { type: String, required: true },
  };

  declare entryIdValue: string;
  declare entryCollectionValue: string;
  ...
  private async loadCard(): Promise<void> {
    ...
    try {
      const response = await fetch(
        `/garden/entry-card/${encodeURIComponent(this.entryCollectionValue)}/${encodeURIComponent(this.entryIdValue)}`
      );
      ...
      console.error(`Error loading card for ${this.entryIdValue}:`, error);
      this.element.innerHTML = `
