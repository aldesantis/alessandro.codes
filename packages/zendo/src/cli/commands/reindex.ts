...
import type { EntryIndexRecord } from "../../config";
 ...
 */
async function parseAllMarkdownFiles(contentDir: string): Promise<FileData[]> {
  ...
  const entries = await ui.withSpinnerAndItemProgress(
    ...
    async (file, index, updateProgress) => {
      const entry = await parseMarkdownFile(path.join(contentDir, file), file);
 ...
 */
export default async function indexContent(config: ResolvedConfiguration): Promise<void> {
  const filesData = await parseAllMarkdownFiles(config.contentDir);
  ...
  await writeJsonFile(getIndexFilePath(config.indexPath), indexEntriesWithAllLinks);
