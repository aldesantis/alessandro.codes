...
import type { Source, SourceResult } from "../../sources";
...
async function fetchContent(
  ...
  const cacheDir = getSourceCacheDir(sourceId, cacheDirName);
...
export default async function fetchAndTransformContent(config: ResolvedConfiguration): Promise<void> {
  ...
  for (const sourceConfig of config.sources) {
    ...
    const { cacheDir: cachePath } = await ui.withSpinnerAndProgress(
      ...
      (updateText) => {
        ...
        return fetchContent(sourceConfig.id, sourceConfig.source, config.cacheDir, wrappedUpdateText);
      },
