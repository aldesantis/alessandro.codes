...
/**
 * Resolves the configured index file path against the current working directory.
 */
export function getIndexFilePath(indexPath: string): string {
  return path.isAbsolute(indexPath) ? indexPath : path.join(process.cwd(), indexPath);
}
