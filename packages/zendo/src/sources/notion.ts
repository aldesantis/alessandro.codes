...
// Types only — the runtime implementations are dynamically imported inside the
// source factory so that `@notionhq/client` / `notion-to-md` remain OPTIONAL
// dependencies that Git-only consumers never need to install.
...
type NotionFilter = Parameters<NotionClient["dataSources"]["query"]>[0]["filter"];
...
export default function notionSource(config: NotionConfiguration): Source {
  return async (destination, context) => {
    // Resolved lazily so these stay optional dependencies.
